...
public class AutorJpaController implements Serializable {
    ...
    public AutorJpaController() {
        emf = Persistence.createEntityManagerFactory("Libreria1PU");
    }
    ...
    public Autor buscarPornombre(String nombre) {
        EntityManager em = getEntityManager();
        Autor autor = (Autor) em.createQuery("SELECT x FROM Autor x WHERE x.nombre LIKE '" + nombre + "'").getSingleResult();
        return autor;
    }

    public List<Autor> buscarPornombrevarios(String nombre) {
        EntityManager em = getEntityManager();
        List<Autor> autor = em.createQuery("SELECT x FROM Autor x WHERE x.nombre LIKE '%" + nombre + "%'").getResultList();
